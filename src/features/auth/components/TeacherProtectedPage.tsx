...
function TeacherRoleGate({ children }: { children: React.ReactNode }) {
  ...
  if (loading) {
    return <main className="mx-auto max-w-7xl px-4 py-16 text-lg font-bold text-ink/70">{t("teacher.protected.loading")}</main>;
  }
    ...
    return (
      <main className="mx-auto max-w-3xl px-4 py-16">
        <div className="rounded-3xl border border-ink/8 bg-white p-8 shadow-soft">
          <h1 className="text-3xl font-black text-ink">{t("teacher.protected.title")}</h1>
          <p className="mt-3 leading-7 text-ink/64">{t("teacher.protected.body")}</p>
          <Button className="mt-6" href="/dashboard" variant="secondary">
            {t("teacher.protected.cta")}
          </Button>
        </div>
      </main>
    );
